...
namespace eTeatar.xUnitTest
{
    public class HomeControllerTest
    {
        ...
        /// <summary>
        /// Testiranje da li dobijemo zadnju predstavu u modelu
        /// </summary>
        [Fact]
        public void IndexZadnjaPredstava()
        {
            HomeController homeController = new HomeController(_context, _userManager, _roleManager)
            {
                ControllerContext = _controllerContext
            };

            var result = Assert.IsType<ViewResult>(homeController.Index());
            var model = Assert.IsType<Web.ViewModels.Home.IndexVM>(result.Model);
            Assert.Equal("Predstava1",model.Predstave[0].Naslov);
        }

        [Fact]
        public void IndexZadnjaNovost()
        {
            HomeController homeController = new HomeController(_context, _userManager, _roleManager)
            {
                ControllerContext = _controllerContext
            ...
            var result = Assert.IsType<ViewResult>(homeController.Index());
            var model = Assert.IsType<Web.ViewModels.Home.IndexVM>(result.Model);
            Assert.Equal("Obavijest1", model.GlavnaNovost.Naslov);
        }

        [Fact]
        public void IndexLogiranUser()
        {
            HomeController homeController = new HomeController(_context, _userManager, _roleManager)
            {
                ControllerContext = _controllerContext
